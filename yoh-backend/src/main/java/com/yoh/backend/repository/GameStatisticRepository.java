...
public class GameStatisticRepository {
    ...
    public List<?> sdsasdew(GamePatient gamePatient) {
        Session session = sessionFactory.openSession();
        try {
            String sql = String.format("select array_agg(\"id\") as \"ids\" from \"started_games\" where \"game_patient_id\" = '%s' and \"date_end\" is not null order by \"date_end\";", gamePatient.getId());
